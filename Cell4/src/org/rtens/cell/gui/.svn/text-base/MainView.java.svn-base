...
public class MainView extends JFrame {
	...
	public static void main(String[] args) {
		try {
			File cellFolder = (args.length == 0 ? new File(".") : new File(args[0]));
	...
	public MainView(File cellFolder, String[] args) throws Exception {
		...
		Messenger.listener = new MessengerListener(rootCell);
		...
		if (args.length == 2) {
			...
			Server socketServer = Connections.getServer("Socket", rootCell);
	...
	private void buildMenu() {
		...
		for (Server s : Connections.getServers(rootCell)) {
		...
		viewMenu.add(new JMenuItem(new AbstractAction("New browser") {
			...
			@Override
			public void actionPerformed(ActionEvent arg0) {
				Browser browser = new Browser(rootCell);
	...
	private class ServerMenu extends JMenu implements ChangeListener {
		...
		private void buildView() {
			...
			for (String paramName : server.getParameterNames()) {
			...
			logFrame = new JFrame("Log of " + server.getNetwork());
			logScrollPane = new JScrollPane(logTextArea);
			logFrame.add(logScrollPane);
			logFrame.setSize(400, 200);
			
			server.setLogger(new Server.Logger() {
